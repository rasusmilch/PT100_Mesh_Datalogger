...
def _pick_time_source(
    df: pd.DataFrame,
) -> Tuple[pd.DataFrame, str, Optional[datetime.tzinfo], int]:
    """Choose the best available time source and create a '__time' column.

    Preference:
      1) iso8601_local if it has at least one parseable timestamp.
      2) epoch_utc (>0) otherwise.

    If both are present and iso8601_local is parseable, we still backfill missing/blank
    iso8601_local values with epoch_utc (converted to the same timezone as the iso values)
    so we don't silently drop valid samples.

    Returns: (filtered_df, time_column_name, tzinfo, dropped_no_time_rows)
    """
    original_len = len(df)
    ...
    if "epoch_utc" in df.columns:
        ...
        # epoch_utc of 0 is used in your logs to indicate unknown time.
    ...
    if iso_series is not None and iso_series.notna().any():
        ...
        if epoch_series is not None and epoch_series.notna().any():
            # Backfill missing iso times with epoch-derived times, converted to iso tz if possible.
            if tzinfo is not None:
                epoch_converted = epoch_series.dt.tz_convert(tzinfo)
            else:
                # If iso is naive, drop tz from epoch and keep it naive.
                epoch_converted = epoch_series.dt.tz_localize(None)

            combined_time = iso_series.copy()
            combined_time = combined_time.fillna(epoch_converted)
        else:
            combined_time = iso_series

    elif epoch_series is not None and epoch_series.notna().any():
        # Fall back to epoch-derived UTC timestamps.
        tzinfo = datetime.timezone.utc
        combined_time = epoch_series
    else:
        raise ValueError(
    ...
    return df, "__time", tzinfo, dropped_no_time_rows
...
def _load_csv_files(file_paths: List[str]) -> LoadedLog:
    ...
    combined, time_column, tzinfo, dropped_no_time_rows = _pick_time_source(combined)
