                                   ...
                                   cal_point_print_summary_t* summary_out)
{
  ...
  summary_out->captured_raw_temp_avg_available = (point->raw_avg_mC != INT32_MIN);
  ...
  summary_out->captured_raw_res_stddev_available = (point->raw_stddev_mOhm >= 0);
  if (summary_out->captured_raw_res_stddev_available) {
    summary_out->captured_raw_res_stddev_ohm = point->raw_stddev_mOhm / 1000.0;
  }
  summary_out->ideal_ref_res_available =
    ComputeIdealReferenceResistanceOhm_(reference_c, &summary_out->ideal_ref_res_ohm);
  ...
  if (summary_out->captured_raw_temp_avg_available) {
    summary_out->residual_temp_c = reference_c - summary_out->captured_raw_temp_avg_c;
                                 ...
                                 bool point_is_resistance_domain)
{
  ...
  if (summary->ideal_ref_res_available) {
    snprintf(ideal_res_buf, sizeof(ideal_res_buf), "%.3f", summary->ideal_ref_res_ohm);
  }
  ...
  if (summary->residual_res_available) {
    snprintf(residual_res_buf, sizeof(residual_res_buf), "%.3f", summary->residual_res_ohm);
  }
  ...
  if (summary->residual_temp_available) {
    snprintf(residual_temp_buf, sizeof(residual_temp_buf), "%.3f", summary->residual_temp_c);
  }
                              ...
                              const runtime_state_t* state)
{
  ...
  for (size_t index = 0; index < settings->calibration_points_count; ++index) {
    ...
    PrintCalibrationPointStatusLine_(point,
                                     &summary,
                                     (uint32_t)(index + 1),
                                     point_is_resistance_domain);
  }
  ...
  if (settings->calibration_points_count == 0u) {
    ...
    for (size_t index = 0; index < settings->calibration_points_count; ++index) {
      ...
      BuildCalibrationPointPrintSummary_(point, &summary);
      const bool point_is_resistance_domain = (point->raw_avg_mOhm > 0);
      PrintCalibrationPointReportBlock_(point,
                                        &summary,
                                        (uint32_t)(index + 1),
                                        point_is_resistance_domain);
    }
