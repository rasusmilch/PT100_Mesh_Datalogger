...
{
  if (state == NULL) {
    ets_printf("runtime: unavailable\n");
    return;
  }
  ...
  ets_printf("markers: storage=%" PRIu32 " sd=%" PRIu32 " uptime_ms=%"
             PRIu32 " reset_reason=%u\n",
             state->storage_marker,
             state->sd_flush_marker,
             uptime_ms,
             (unsigned)reset_reason);
  ets_printf("fram: count=%" PRIu32 "/%" PRIu32 " wm=%" PRIu32 "\n",
             cached->fram_count,
             cached->fram_capacity,
             cached->fram_flush_watermark_records);
  ets_printf("heap: int_free=%" PRIu32 " int_largest=%" PRIu32
